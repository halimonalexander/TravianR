        ...
        if (mysql_num_rows($MyGold)) {
            if ($golds['6'] > 2) {
                ...
                if (mysql_num_rows($MyGold)) {
                    ...
                    if ($golds['b3'] < time()) {
                        mysql_query("UPDATE " . TB_PREFIX . "users set b3 = '" . (time() + PLUS_PRODUCTION) . "' where `id`='" . $session->uid . "'");
                    } else {
                        mysql_query("UPDATE " . TB_PREFIX . "users set b3 = '" . ($golds['b3'] + PLUS_PRODUCTION) . "' where `id`='" . $session->uid . "'");
                    }


                    $done1 = "+25% Production: Iron";
                    mysql_query("UPDATE " . TB_PREFIX . "users set gold = " . ($session->gold - 5) . " where `id`='" . $session->uid . "'");
                    mysql_query("INSERT INTO " . TB_PREFIX . "gold_fin_log VALUES ('" . (mysql_num_rows($goldlog) + 1) . "', '" . $village->wid . "', '+25%  Production: Iron')");

                } else {
                    $done1 = "nothing has been done";
                    mysql_query("INSERT INTO " . TB_PREFIX . "gold_fin_log VALUES ('" . (mysql_num_rows($goldlog) + 1) . "', '" . $village->wid . "', 'Failed +25%  Production: Iron')");
