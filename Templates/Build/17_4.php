<?php if ($session->goldclub == 1 && count($database->getProfileVillages($session->uid)) > 1) { ?>
    ...
    if (isset($_GET['create']) && $session->gold > 1) {
        ...
        <p><?php echo TRADE_ROUTES_DESC; ?> <img src="../../<?php echo GP_LOCATE; ?>img/a/gold.gif" alt="Gold"
                                                 title="<?php echo GOLD; ?>">2.</p>
        ...
        <table id="npc" cellpadding="1" cellspacing="1">
            ...
            if (count($routes) == 0) {
                ...
                foreach ($routes as $route) {
                    ?>
                    <tr>
                        <th><label><input class="radio" type="radio"
                                          onclick="window.location.href = '?id=<?php echo $id; ?>&t=4&routeid=<?php echo $route['id']; ?>';"
                                          name="routeid"
                                          value="<?php echo $route['id']; ?>" <?php if ($routeid == $route['id']) {
                                    echo "" . CHECKED . "";
                                } ?>></label></th>
                        <th>
                            <?php
                        ...
                        <th><?php echo ceil(($route['timeleft'] - time()) / 86400);
                            echo " " . DAYS . ""; ?></th>
                    </tr>
                <?php }
            } ?>
        ...
        * <?php echo EXTEND_TRADE_ROUTES; ?> <img src="../../<?php echo GP_LOCATE; ?>img/a/gold.gif" alt="Gold"
                                                  title="<?php echo GOLD; ?>">2
        <br>
        <div class="options">
