...
switch ($kind) {
    case 1:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = BUILDING_WEAKER;
        } else {
            $betterorbadder = BUILDING_STRONGER;
        }
        break;
    case 2:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = TROOPS_SLOWEST;
        } else {
            $betterorbadder = TROOPS_FASTER;
        }
        break;
    case 3:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = SPIES_DECRESE;
        } else {
            $betterorbadder = SPIES_INCREASE;
        }
        break;
    case 4:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = CONSUME_HIGH;
        } else {
            $betterorbadder = CONSUME_LESS;
        }
        break;
    case 5:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = TROOPS_MAKE_SLOWEST;
        } else {
            $betterorbadder = TROOPS_MAKE_FASTER;
        }
        break;
    case 6:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = YOU_CONSTRUCT;
        } else {
            $betterorbadder = YOU_CONSTRUCT;
        }
        break;
    case 7:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
            $betterorbadder = CRANNY_DECRESE;
        } else {
            $betterorbadder = CRANNY_INCREASED;
        }
        break;
    case 8:
        if ($artefact['type'] == 8 && $artefact['bad_effect'] == 1) {
...
<div class="artefact image-6">
    ...
    <table class="art_details" cellpadding="1" cellspacing="1">
        ...
        <tr>
            <td><span class="none"><a
                            href="spieler.php?uid=<?php echo $artefact['owner']; ?>"><?php echo $database->getUserField($artefact['owner'], "username", 0); ?></a></span>
            </td>
            <td><span class="none"><a
                            href="karte.php?d=<?php echo $artefact['vref']; ?>&c=<?php echo $generator->getMapCheck($artefact['vref']); ?>"><?php echo $database->getVillageField($artefact['vref'], "name"); ?> </a></span>
            </td>
