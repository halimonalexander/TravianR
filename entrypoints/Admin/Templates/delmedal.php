...
$sql = mysql_query("SELECT * FROM medal");
...
<table id="profile">
    ...
    $sql = mysql_query("SELECT week FROM medal ORDER BY week DESC LIMIT 1");
...
<form action="../GameEngine/Admin/Mods/deletemedalbyweek.php" method="POST">
    ...
    <table id="profile">
        ...
        for ($j = 0; $j < $week; $j++) {
            ...
            $sql = mysql_query("SELECT * FROM medal WHERE week = $newweek");
            $tot = mysql_num_rows($sql);
...
<table id="profile">
    ...
    <tbody>
    <?php
    $query = "SELECT * FROM medal ORDER BY id DESC";
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        switch ($row['categorie']) {
            ...
            case "4":
                ...
                break;
            case "5":
                $titel = "Top 10 Att and Def";
                break;
            case "6":
                $titel = "Top 3 Att, " . $medal['points'] . " in a row";
                break;
            case "7":
                $titel = "Top 3 Def," . $medal['points'] . " in a row";
                break;
            case "8":
                $titel = "Top 3 Climber, " . $medal['points'] . " in a row";
                break;
            case "9":
                $titel = "Top 3 Robber, " . $medal['points'] . " in a row";
                break;
            case "10":
            ...
            case "11":
                ...
                break;
            case "12":
                $titel = "Top 10 Attacker, " . $medal['points'] . " in a row";
                break;
        }
        $title = $titel;
        $rank = $row['plaats'];
        ...
        $unq = "SELECT username FROM users where id = $playerid";
