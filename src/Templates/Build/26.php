...
if ($_POST and $_GET['action'] == 'change_capital') {
    ...
    if ($data['password'] == md5($pass)) {
        ...
        if ($data2['vref'] != $village->wid) {
            for ($i = 1; $i <= 18; ++$i) {
                if ($data2['f' . $i] > 10) {
                    $query2 = mysql_query('UPDATE `fdata` SET `f' . $i . '` = 10 WHERE `vref` = ' . $data2['vref']);
                }
            }
            for ($i = 19; $i <= 40; ++$i) {
                if ($data2['f' . $i . 't'] == 34) {
                    $query3 = mysql_query('UPDATE `fdata` SET `f' . $i . 't` = 0, `f' . $i . '` = 0 WHERE `vref` = ' . $data2['vref']);
                }
            }

            for ($i = 19; $i <= 40; ++$i) {
                if ($data2['f' . $i . 't'] == 29 or $data2['f' . $i . 't'] == 30 or $data2['f' . $i . 't'] == 38 or $data2['f' . $i . 't'] == 39 or $data2['f' . $i . 't'] == 42) {
                    $query3 = mysql_query('UPDATE `fdata` SET `f' . $i . 't` = 0, `f' . $i . '` = 0 WHERE `vref` = ' . $village->wid);
                }
            }
