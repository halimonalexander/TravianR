...
/////////////////////////////////////////////////////////
    ...
    $accountCheck = mysql_query("SELECT * FROM users WHERE `id`='" . $session->uid . "'");
    ...
    $plusCheck = mysql_query("SELECT * FROM users WHERE `id`='" . $session->uid . "'");
    ...
    if (mysql_num_rows($plusCheck) > 0) {
        if ($time > $pluss[$key]) {
            $editplus = mysql_query("UPDATE users SET `{$key}`= `{$key}` + ('" . $time . "'+'" . $plusTime . "'),  `gold` =  `gold` - {$gldz}   WHERE `id`='" . $session->uid . "'");
            ...
            if ($time < $pluss[$key]) {
                $editplus = mysql_query("UPDATE users SET `{$key}`= `{$key}` +'" . $plusTime . "',  `gold` =  `gold` - {$gldz}  WHERE `id`='" . $session->uid . "'");
                echo "<META HTTP-EQUIV=Refresh CONTENT=\"2; {$url}\" ><br /><br /><div align=center><font color=green size=4><b> Your Status has been updated!</b></font></div>";

            }
    } else {
        $insertplus = mysql_query("INSERT INTO users (`username`,`{$key}`, `gold`) VALUES ('" . $session->username . "', ('" . $time . "'+'" . $plusTime . "'),`gold` - {$gldz})");
