        ...
        if (mysql_num_rows($MyGold)) {
            if ($golds['6'] > 2) {
                ...
                if (mysql_num_rows($MyGold)) {
                    ...
                    if ($golds['b2'] < time()) {
                        mysql_query("UPDATE users set b2 = '" . (time() + PLUS_PRODUCTION) . "' where `id`='" . $session->uid . "'");
                    } else {
                        mysql_query("UPDATE users set b2 = '" . ($golds['b2'] + PLUS_PRODUCTION) . "' where `id`='" . $session->uid . "'");
                    }


                    $done1 = "+25% Production: Clay";
                    mysql_query("UPDATE users set gold = " . ($session->gold - 5) . " where `id`='" . $session->uid . "'");
                    mysql_query("INSERT INTO gold_fin_log VALUES ('" . (mysql_num_rows($goldlog) + 1) . "', '" . $village->wid . "', '+25%  Production: Clay')");

                } else {
                    $done1 = "nothing has been done";
                    mysql_query("INSERT INTO gold_fin_log VALUES ('" . (mysql_num_rows($goldlog) + 1) . "', '" . $village->wid . "', 'Failed +25%  Production: Clay')");
